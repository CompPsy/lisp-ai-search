...
;;;     - parameters: an xy pair that denotes the height and width of the search gride
;;;     - agent-location: an xy pair that denotes the current location of the agent in the world
;;; Description: 
;;;     This class creates the world that the search agent will be excuting actions in
;;; Author: Elizabeth Thompson
;;; Created 3/2/2019
;;; Modications: none
;;;
...
(defparameter *world*
  (make-instance 'world))
(setf (parameters *world*) (cons 26 26))
...
;;; Created 3/5/2019
;;; Modications: none
(defun make-nodes-list (world)
    (let (a b)
        (setf a (- (car (parameters world)) 1))
        (setf b (- (cdr (parameters world)) 1))
        (setf (nodes-loc *world*) (loop for n from 0 to a
                                   append (loop for m from 0 to b
                                        collect (cons n m))))))

;;>>> probably better as a method, just for clarity:

;;>>>     (defmethod make-nodes-list ((self world))
;;>>>       (with-slots (parameters nodes-loc) self
;;>>>         (setq nodes-loc 
;;>>>           (loop for n from 0 to (1- (car parameters))
;;>>>               append (loop for m from 0 to (1- (cdr parameters)))
;;>>>                      collect (cons n m)))))
...
;;; Class: agent
;;; Slots: 
;;;     - agent-x: x coordinate of the agent
;;;     - agent-y: y coordinate of the agent
;;;     - facing-dir: stores a letter denoting the direction the agent is facing
;;; Description: 
...
;;; Class: reflex-agent
;;; Slots: 
;;;     - sensors: various sensors that denote the ability of the agent to detect the world around it
;;;     - corner-found: flag for if the agent has found a corner
;;; Description: 
;;;     This class inherits from agent class and includes necessary sensors and checks for a reflex agent
;;;     including that a reflex agent has no knowledge of the world outside of its sensors.
;;; Author: Elizabeth Thompson
;;; Created 3/3/2019
;;; Modications: none
...
;;;     - corner-found: flag for if the agent has found a corner
;;; Description: 
;;;     This class inherits from agent class and includes necessary sensors and checks for a model-reflex agent
;;;     including the ability to store visited locations data.
;;; Author: Elizabeth Thompson
;;; Created 3/4/2019
;;; Modications: none
(defclass model-agent (agent) (  ;;does it want to store the knowledge in a graph-node-edge format?
    (front-sensor :initform nil :accessor front-sensor)
    (front-bump :initform nil :accessor front-bump)
    (left-bump :initform nil :accessor left-bump)
    (back-bump :initform nil :accessor back-bump)
...
;;; Description: 
;;;     This class inherits from agent class and includes necessary sensors and checks for an HillClimbing agent
;;;     including the ability to store visited locations data, and calculate the distance from the current node to
;;;     the goal node.
;;; Author: Elizabeth Thompson
;;; Created 3/5/2019
;;; Modications: none
...
;;; Created 3/6/2019
;;; Modications: none
(defmethod update-precept (agent precept)
